...
DEFAULT_PREDICTOR_PATH = os.path.expanduser(os.path.join(
    '~',
    'Library',
    'dlib',
    'examples',
...
def largest_face(face_bounds_list):
    ...
    for bounds in face_bounds_list:
        face_size = (bounds.left()-bounds.right())**2 + \
...
for file_name in ARGS.images:
    ...
    # Detect bounds of faces, 2nd arg means upscale 1 time(s) to help detection
    dets = DETECTOR(img, 1)
    if ARGS.verbose:
...
SM_TFORM = tr.SimilarityTransform(translation=(-LEFT+WIDTH/2, -TOP+HEIGHT/2))
MEAN_SHAPE = np.array(SM_TFORM(MEAN_SHAPE))
BOTTOM = max([y for _, y in MEAN_SHAPE])
RIGHT = max([x for x, y in MEAN_SHAPE])
CORNERS = np.array([list(xy)
                    for xy
                    in zip([0]*2+[WIDTH*2-1]*2, [HEIGHT*2-1, 0]*2)])
...
for img, face, shape in FACE_LIST:
    ...
    affine_img = tr.warp(img, af_tform.inverse,
                         output_shape=(int(WIDTH*2), int(HEIGHT*2)))
    shape_points = np.array(af_tform(shape_points))
    # Perform a peicewise affine transform from the face shape to the
    # mean_face shape i.e. warp the face image to match the average shape
    ...
    warped = tr.warp(affine_img, pw_af_tform.inverse,
                     output_shape=(int(WIDTH*2), int(HEIGHT*2)))
